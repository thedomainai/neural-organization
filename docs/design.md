...
Perception が「現実世界への接続」を担うとき、具体的には以下の 7 カテゴリのツール群との接続を意味する。Hutchins (1995) の分散認知が示すように、認知は環境に分散する。7 カテゴリのツール接続は、組織の認知を現実世界に分散させる具体的な実装である。
...
Understanding が構築する「世界モデル」は、グラフ構造のエンティティとして実装される。Friston (2010) の予測的符号化に基づき、世界モデルは静的なデータベースではなく、予測と現実の誤差を駆動力として動的に更新される生成モデルである。以下に Customer エンティティの設計例を示す。
...
> 詳細: [philosophy/layer1-understanding-design.md](philosophy/layer1-understanding-design.md)

### 1.3 Layer 2: Reasoning（推論）— 領域横断構成の具体例

Reasoning の核心である「領域横断的な統合行動」がどのように機能するかを、定量的な例で示す。Thompson (1967) の相互依存性理論と Malone & Crowston (1994) のコーディネーション理論が示すように、組織の価値は部門間の依存関係を統合的に管理することで生まれる。Kahneman の二重過程理論に基づき、既知パターンへの高速対応（System 1 的）と未知状況での分析的推論（System 2 的）を使い分ける。
...
> 詳細: [philosophy/layer2-reasoning-design.md](philosophy/layer2-reasoning-design.md)

### 1.4 Layer 3: Execution（実行）— アーティファクトと品質保証

Smith (1980) の Contract Net Protocol に基づき、タスクは能力による間接参照で適切な実行者に委任される。Woods (2018) の graceful extensibility が示すように、実行系は想定外の負荷にも段階的に能力を拡張できる設計を指向する。
...
Execution は出力前に 4 段階の品質チェックを行い、Governance Gate に到達する前にシステム内で品質を担保する（二重フィルタリング原則の実装）。この段階的チェックは Hollnagel (2011) のレジリエンスの 4 つの礎石（respond, monitor, learn, anticipate）と Deming の PDCA サイクルの知見に基づく。品質は事後検査ではなくプロセスに組み込むべきであり、段階ごとに異なる観点で検証することで、単一チェックでは見落とす問題を捕捉する。
...
concept.md で定義した 5 種類の学習入力に加え、内省が各層にどのような具体的な更新を生むかを示す。Argyris (1977) のダブルループ学習が示す「前提そのものの修正」と、Flavell (1979) のメタ認知（自己の認知プロセスを認知する能力）が、Layer 4 の自己参照的内省の学術的基盤である。
...
concept.md で定義した Purpose の 4 要素（Why / Where / How / Boundary）が、Reasoning の行動計画導出にどう作用するかを具体例で示す。Ostrom (1990) の立憲的統治における constitutional rules（ルールを作るルール）に相当し、個々の運用判断を方向づける上位の意志を構造化する。
...
> 詳細: [philosophy/purpose-design.md](philosophy/purpose-design.md)

### 2.2 Governance（統治）— 信頼スコアモデル

Davis et al. (1997) のスチュワードシップ理論が示す信頼に基づく委任を、定量的に実装するモデルである。Hersey & Blanchard (1977) の状況適応型リーダーシップが「部下の成熟度に応じて委任範囲を拡大する」ことを説いたように、信頼スコアに応じて自律範囲を動的に調整する。Mayer, Davis & Schoorman (1995) の信頼の統合モデル（能力・誠実さ・善意の 3 因子）は、スコアの構成要素設計の参照とした。
...
concept.md で定義した 3 つの記憶構造（長期記憶・作業記憶・評価記憶）それぞれに適切な技術スタックを対応させる。Laird, Lebiere & Rosenbloom (2017) の Standard Model of the Mind が同定した 3 構造記憶（宣言的長期記憶・バッファ型作業記憶・手続き記憶）を組織スケールに写像した設計である。
...
> 詳細: [philosophy/memory-design.md](philosophy/memory-design.md)

### 2.4 Orchestration（統率）— フロー制御・リソース配分・優先度管理

Wiener (1948) のサイバネティクスが定式化したフィードバック制御と、Ashby (1956) の必要多様性の法則が理論的基盤である。4 つのフローパターンと 3 つのリソース配分戦略の組み合わせにより、組織が直面する多様な状況に対して十分な制御の多様性を確保する。
...
> 詳細: [philosophy/interface-design.md](philosophy/interface-design.md)

## 4. 学術的基盤

本設計ドキュメントで参照した主要な学術的根拠を整理する。詳細な検証は [references/academic-validation.md](../references/academic-validation.md) を参照。

### 4.1 レイヤー設計の学術的根拠

| 層 | 学術的根拠 | 設計への反映 |
|---|---|---|
| L0 Perception | Hutchins (1995) 分散認知 | 7 カテゴリのツール接続による認知の環境分散 |
| L1 Understanding | Friston (2010) 予測的符号化 | 予測と現実の誤差駆動で更新される動的世界モデル |
| L2 Reasoning | Kahneman 二重過程理論、Thompson (1967) 相互依存性、Malone & Crowston (1994) コーディネーション | 定型/創発の二刀流推論、領域横断的な統合行動計画 |
| L3 Execution | Smith (1980) CNP、Woods (2018) graceful extensibility | 能力ベースのタスク委任、段階的能力拡張 |
| L4 Reflection | Argyris (1977) ダブルループ学習、Flavell (1979) メタ認知 | 前提そのものの修正を含む全層更新 |

### 4.2 品質保証・フィードバック設計の学術的根拠

| 設計要素 | 学術的根拠 | 設計への反映 |
|---|---|---|
| 品質保証の 4 段階チェック | Hollnagel (2011) レジリエンスの 4 つの礎石（respond, monitor, learn, anticipate） | 段階的・多観点の品質検証プロセス |
| 段階的品質組み込み | Deming PDCA サイクル | 品質を事後検査ではなくプロセスに組み込む設計 |
| 二重フィルタリング原則 | Simon (1955) 限定合理性、Miller (1956) 認知容量制約 | 人間の注意を最もレバレッジの高い判断に集中 |

### 4.3 横断的要素の学術的根拠

| 要素 | 学術的根拠 | 設計への反映 |
|---|---|---|
| Purpose | Ostrom (1990) 立憲的統治 | 運用ルールの上位にある constitutional rules としての意志定義 |
| Governance | Davis et al. (1997) スチュワードシップ理論、Hersey & Blanchard (1977) 状況適応型リーダーシップ、Mayer et al. (1995) 信頼の統合モデル | 信頼ベースの委任、成熟度に応じた自律範囲の動的調整 |
| Memory | Laird et al. (2017) Standard Model of the Mind の 3 構造記憶 | 長期記憶・作業記憶・評価記憶の 3 構造設計 |
| Orchestration | Wiener (1948) サイバネティクス、Ashby (1956) 必要多様性の法則 | フィードバック制御と多様な制御パターンによる状況対応 |
