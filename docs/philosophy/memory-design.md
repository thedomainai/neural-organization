...
Memory の 3 構造は、認知科学における記憶の分類と構造的に対応する。

**心の標準モデルとの対応**: Laird, Lebiere & Rosenbloom (2017) が提唱した「心の標準モデル（Standard Model of the Mind）」は、3 つの独立した認知アーキテクチャ（Soar, ACT-R, Sigma）が 40 年の研究を経て収斂した共通構造である。このモデルは宣言的長期記憶（知識の蓄積）、バッファ型作業記憶（進行中タスクの文脈保持）、手続き記憶（行動のパターン化）の 3 構造を基本とする。Neural Organization の長期記憶・作業記憶・評価記憶はこの 3 構造と対応関係にある。

**エピソード記憶と意味記憶の区別**: Tulving (1972) は記憶を意味記憶（一般化された知識・事実）とエピソード記憶（特定の出来事とその文脈）に区別した。この分類に照らすと、長期記憶は意味記憶に対応する — 個別の事例から抽象化された組織の知恵（成功パターン、判断基準、因果モデル）を保持する。評価記憶はエピソード記憶に対応する — 個別の行動とその結果（いつ・何をして・どうなったか）を文脈とともに保持する。

**オートポイエーシスとしての Memory**: Maturana & Varela (1980) のオートポイエーシス理論は、生命システムが自己を産出し維持する特性を記述する。Memory は組織の自己維持的な知識基盤として機能し、人員の変動という外部擾乱に対しても組織の同一性（パターン、価値観、判断基準）を保全し続ける。記憶の階層的抽象化（生のイベント → パターン → 原則 → 価値観）は、この自己産出プロセスの実装である。
...
**工学的先例 — Event Sourcing パターン**: この Append-Only 原則は、ソフトウェアアーキテクチャにおける Event Sourcing パターンの直接的な適用である。Event Sourcing では、システムの状態変化を不変のイベントログとして記録し、任意の時点の状態を再構成可能にする。不変のイベントログは監査証跡（audit trail）として機能し、「何が・いつ・なぜ起きたか」の完全な履歴を保証する。Neural Organization の Memory における追記専用ログ、イベントソーシングによるバージョニング、実行トレースの構造化記録は、いずれもこのパターンの実装である。
...
Memory の設計は、認知科学・生物学・ソフトウェア工学の知見に基づいている。

| 設計要素 | 学術的根拠 | 対応関係 |
|---|---|---|
| 3 つの記憶構造（長期・作業・評価） | Laird, Lebiere & Rosenbloom (2017) 心の標準モデル — 宣言的長期記憶、バッファ型作業記憶、手続き記憶 | 3 つの独立した認知アーキテクチャが 40 年で収斂した共通構造 |
| 長期記憶 = 一般化された知識 | Tulving (1972) 意味記憶 — 特定の出来事から独立した一般的知識 | 成功パターン・判断基準・因果モデルは意味記憶に相当 |
| 評価記憶 = 出来事と結果の記録 | Tulving (1972) エピソード記憶 — 特定の出来事とその文脈 | 行動-結果ペア、KPI 推移、予測と実績の乖離はエピソード記憶に相当 |
| Append-Only・イベントソーシング | Event Sourcing パターン — 不変のイベントログによる状態管理 | 追記専用ログ、実行トレース、バージョニングの工学的先例 |
| 人員変動を超えた知識保全 | Maturana & Varela (1980) オートポイエーシス — 自己を産出し維持するシステム | Memory が組織の自己維持的な知識基盤として機能する |
