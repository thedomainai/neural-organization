...
ツール接続設計は、マルチエージェントシステム・サイバネティクス・認知科学の知見に基づいている。

**Contract Net Protocol と能力ベースの抽象化**: Smith (1980) の Contract Net Protocol は、タスクを能力要件として公告し、能力を持つエージェントが応札する動的なタスク配分モデルである。Neural Organization のツール接続設計は、この着想を情報取得に応用している — 特定のツール名ではなく「組織が知覚すべき現実の次元」（状態・活動・関係・変化）を定義し、その次元を満たす能力を持つツールを接続する。原則 4（ツールは手段であり目的ではない）は Contract Net の能力ベース抽象化と同型である。

**必要多様性の法則と 7 カテゴリ**: Ashby (1956) の必要多様性の法則は、制御システムが環境の多様性に匹敵する多様性を持つ必要があることを示す。7 カテゴリのツール群（コミュニケーション、顧客関係、プロダクト開発、マーケティング、人事組織、ファイナンス、運用・インフラ）は、組織の活動領域の多様性をカバーするための設計である。これらのカテゴリが不足すると、組織の現実の一部が「盲点」となり、能動的接続判断のトリガーとなる。

**分散認知**: Hutchins (1995) の分散認知理論は、認知プロセスが個人の頭の中に閉じるのではなく、人間・道具・環境にわたって分散することを示した。Neural Organization のツール群は分散認知システムの構成要素であり、各ツールが組織の認知の一部を担う — Slack が意思決定の記録を、Salesforce が顧客関係の状態を、GitHub が開発活動の進捗を保持する。Perception レイヤーの役割は、これらの分散した認知を単一の世界モデルに統合することにある。
...
| 設計要素 | 学術的根拠 | 対応関係 |
|---|---|---|
| ツールを能力要件で抽象化する設計 | Smith (1980) Contract Net Protocol — 能力ベースの動的タスク配分 | 特定ツールではなく「知覚すべき次元」で定義し、能力を持つツールを接続する |
| 7 カテゴリによる組織活動の網羅 | Ashby (1956) 必要多様性の法則 — 多様性のみが多様性を吸収する | 組織活動の多様性をカバーするカテゴリ設計。不足は盲点となる |
| ツール群を通じた組織の認知 | Hutchins (1995) 分散認知 — 認知は人間・道具・環境に分散する | 各ツールが組織の認知の一部を担い、Perception が統合する |
