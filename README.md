...
このプロジェクトが目指すアウトプットには、3つの軸がある。

| # | アウトプット | 手段 | 動機 |
|---|---|---|---|
| 1 | **思想の完成** | `.md` ファイル群 | 知的好奇心 — 「組織の次の形態」を自分自身が納得いくまで言語化する |
| 2 | **思想のビジュアル化** | Web サイト | 承認欲求 — 考えたことを世界に伝え、共鳴する人・批判する人と出会う |
| 3 | **プロダクト** | Web アプリ | 事業化 — 思想を実際の組織に届け、価値を証明する |

これら3つは段階的に積み上がる関係にある。**思想が土台になければサイトは空虚になり、サイトで伝わらなければプロダクトは使われない。**
現時点では1の完成度を高めることが最優先であり、2・3はその延長線上に位置する。

### Done 定義

| # | アウトプット | Done の条件 |
|---|---|---|
| 1 | 思想の完成 | concept.md / design.md / framework.md が相互に矛盾なく、6つの設計原理を一貫して説明している。philosophy/ の全10ファイルが記述完了し、各層・横断要素の設計が具体例で検証可能。第三者が docs/ を通読して Neural Organization を自分の言葉で要約・批判できる明晰さに到達している |
| 2 | 思想のビジュアル化 | LP が独自ドメインでデプロイされ、Customer / Investor 両バリアントがアクセス可能。月間 100 UU 以上のオーガニックトラフィック。Contact Form 経由で月間 5 件以上のリードを獲得 |
| 3 | プロダクト | Neumann が β顧客 3-5 社で Phase 1a（接続と信頼構築）を完了。Trust Score が 1 社以上で 0.50 以上に到達。Phase 2（パターン学習）への移行判定が可能な状態 |
...
```
        ...
        │              Purpose（目的）               │
        ├─────────────────────────────────────────┤
        │  L0 Perception（検知）→ L1 Understanding（解釈）→     │
...
このサイトとプロダクトが届けたい相手を以下に定義する。

### Customer LP（`/` メインページ）

**プライマリ: テクノロジー企業の経営者（CEO / COO / CTO）、50〜500人規模**

- AI 活用を経営課題として認識しているが、「既存業務に AI ツールを足す」では根本的な変化が得られないと感じている
- 複数の SaaS を導入済みで、データのサイロ化・部門間の意思決定の遅さに課題がある
- 「組織をどう再設計すべきか」という問いに答えを探している層

**セカンダリ: 大企業の DX / AI 推進リーダー**

- 既存組織に AI を組み込む設計論・フレームワークを探している
- 「AI で人を置き換える」ではなく「AI で組織を進化させる」アプローチに共感する

### Investor LP（`/investors`）

- AI / SaaS 領域に投資する VC パートナー
- Vertical SaaS の次のカテゴリとして「自律的組織知能」に関心がある
- 日本発・グローバル展開の可能性を評価する投資家

### 思想の拡散（間接的ターゲット）

- AI × 組織論に関心を持つビジネスリーダー・研究者・エンジニア
- カンファレンス、ブログ、SNS で思想を拡散し、議論を起こしてくれる層
- Neural Organization の思想に共鳴し、または批判的に検討してくれる知識人
