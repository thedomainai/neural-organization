...
以下はレベニューサイクルドメインにおける各層の具体的なマッピング:

| 層 | レベニューサイクルドメインでの責務 |
|---|---|
| **Purpose** | 事業計画: 売上目標 + GTM 戦略 + ICP 定義 + 価値提案 |
| **Governance** | Trust Score × ゲート |
| **L0 Perception** | リードデータ取得（マーケ施策、Web 行動、インバウンド）、CRM パイプラインデータ、プロダクト利用データ、顧客の声（サポートチケット、NPS）、市場データ（ai-exec-dashboard 連携） |
| **L1 Understanding** | リードスコアリング（ICP 適合度 × エンゲージメント）、顧客ジャーニーマッピング、顧客健全性スコアリング、解約リスク予測、プロダクトフィードバック構造化 |
| **L2 Reasoning** | リード優先度判断（LTV/CAC 最適化）、商談戦略立案、提案戦略（フレームワーク選択 + ストーリー構成）、価格戦略、アップセル/クロスセル機会特定、プロダクトロードマップ提案 |
| **L3 Execution** | プレゼン資料生成（← v1 の SPIN 生成）、提案書・見積書生成、アウトリーチメール/メッセージ生成、CS コミュニケーション生成、プロダクトフィードバックレポート生成 |
| **L4 Reflection** | Win/Loss 分析、提案効果測定、CS 施策効果測定、勝ち/負けパターンの自動抽出 → Knowledge 更新、フィードバック→プロダクト反映の追跡 |
| **Memory** | 長期: 勝ち/負けパターン辞書 + 顧客エンティティモデル + ICP プロファイル / 作業: 進行中の商談文脈 / 評価: 受注率推移、解約率推移 |
| **Orchestration** | パイプライン駆動（ステージ変化がトリガー）、CS サイクル（月次/四半期）、アラート駆動（解約リスク検知等） |
