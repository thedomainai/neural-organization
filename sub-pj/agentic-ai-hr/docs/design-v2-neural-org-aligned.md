...
以下は HR 制度設計ドメインにおける各層の具体的なマッピング:

| 層 | HR 制度設計ドメインでの責務 |
|---|---|
| **Purpose** | 企業の Mission/Vision/Values + 3 つの究極の問いへの回答 |
| **Governance** | Trust Score × 7+α ゲート |
| **L0 Perception** | 企業コンテキスト収集（業種、規模、現行制度、課題）、外部データ取り込み（労働市場、法規制、業界標準報酬）、運用データ収集（評価結果、離職率、従業員サーベイ） |
| **L1 Understanding** | 組織能力の構造化（コンピテンシー・トリアドへのマッピング）、現行制度の因果分析（制度設計 ↔ 人材行動 ↔ 組織成果）、業界ベンチマーク比較 |
| **L2 Reasoning** | 求める人材像の設計（コンピテンシーモデリング）、等級制度の設計（スコープベースモデル適用）、評価制度の設計（3 ゲート + 5 段階採点）、報酬制度の設計（3 つの究極の問い） |
| **L3 Execution** | 制度ドキュメント生成（等級定義書、評価シート、報酬テーブル）、導入計画の生成、運用ツール生成、制度説明資料の生成 |
| **L4 Reflection** | HITL 修正パターンの分析 → 暗黙の価値観抽出、制度運用結果の評価、他社制度の成功/失敗パターンの蓄積、制度改定提案の自動生成 |
| **Memory** | 長期: フレームワーク知識 + 組織固有の学習 / 作業: 進行中の設計セッション / 評価: 制度運用の KPI 推移 |
| **Orchestration** | DAG ワークフロー、依存関係解決、HITL 待ち制御、リソース配分 |
