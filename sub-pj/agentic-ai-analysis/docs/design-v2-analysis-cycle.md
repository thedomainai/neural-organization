...
本プロダクトは Neural Organization の 5 層アーキテクチャに完全準拠する。
フレームワークの詳細は [`/docs/neural-organization-5-layer-framework.md`](../../../docs/neural-organization-5-layer-framework.md) を参照。

以下はデータ分析サイクルドメインにおける各層の具体的なマッピング:

| 層 | データ分析サイクルドメインでの責務 |
|---|---|
| **Purpose** | 経営課題: どの問いに分析リソースを投じるか |
| **Governance** | Trust Score × ゲート |
| **L0 Perception** | 分析リクエストの受付（経営層・各部門・他 Agentic AI）、データソースカタログの管理と自動接続、データ品質シグナルの検知（欠損、異常値、スキーマ変更）、外部データの取り込み |
| **L1 Understanding** | データプロファイリング（分布、相関、欠損パターン）、データ品質ルールの適用と自動クレンジング、探索的分析（EDA）、メタデータの構造化 |
| **L2 Reasoning** | 問いの優先度判定（ビジネスインパクト × 実現可能性）、分析設計（手法選択、必要データ、前提条件）、仮説検証（統計的検定、因果推論、予測モデル構築）、インサイト抽出（So What? → Now What?）、アクション提案 |
| **L3 Execution** | SQL / Python コード生成・実行、可視化の生成（グラフ、ダッシュボード、図解）、レポート・プレゼン資料の生成、データパイプラインの構築・更新、アラート・通知の配信 |
| **L4 Reflection** | 分析精度の追跡（予測 vs 実績）、アクション効果の測定、分析手法の有効性評価 → ベストプラクティス更新、データ品質ルールの精度改善、「問い」の質の評価 |
| **Memory** | 長期: 分析ナレッジベース + データ品質ルール辞書 + 分析パターンライブラリ / 作業: 進行中の分析コンテキスト / 評価: 予測精度推移 |
| **Orchestration** | リクエスト駆動（分析依頼をトリガー）、定期駆動（月次/四半期）、アラート駆動（データ異常検知時） |
